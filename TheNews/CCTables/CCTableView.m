...
- (NSUInteger)_CellIndexFromOffset:(CGPoint)offset {
    NSInteger  index;
    CGSize     cellSize;
    ...
    switch (self.direction) {
        case CCScrollViewDirectionHorizontal:
            index = offset.x/(cellSize.width*CC_CONTENT_SCALE_FACTOR());
            break;
        default:
            index = offset.y/(cellSize.height*CC_CONTENT_SCALE_FACTOR());
			//NSLog(@"_indexFromOffset =%d offset.y=%f  cellSize.height = %f",index,offset.y,cellSize.height);
            break;
    }
...
- (void)_moveCellOutOfSight:(CCTableViewCell *)cell {
	[tDelegate_ cellRemoved:self Removed:cell];
    [cellsFreed_ addObject:cell];
...
- (void)_setIndex:(NSUInteger)index forCell:(CCTableViewCell *)cell {
    ...
    item.position    = ccp(offset.x, offset.y);
    cell.idx         = index;
}
#pragma mark -
...
-(void)scrollViewDidScroll:(CCScrollView *)view {
   ...
   // CCLog(@"%s: %s: cells in use: %i", __FILE__, __FUNCTION__, [cellsUsed_ count]);
   // CCLog(@"%s: %s: cells in free list: %i", __FILE__, __FUNCTION__, [cellsFreed_ count]);
    [pool drain];
}
...
-(void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([touches count] == 1 && !self.touchMoved_ &&
        ...
        localPoint = CC_POINTS_TO_PIXELS(localPoint);
		localPoint = ccpAdd(ccp(-localPoint.x, -localPoint.y), self.touchPoint_);
        //NSLog(@"localPoint x=%f  y=%f",localPoint.x, localPoint.y);
		index      = [self _CellIndexFromOffset:localPoint];
